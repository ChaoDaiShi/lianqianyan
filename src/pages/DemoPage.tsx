...
export function DemoPage() {
  ...
  return <AppShell companion={companion}><div className="space-y-6">
    ...
    <DemoStoryStep number={5} title="进入学习空间" icon={PlayCircle} status={startingTaskId ? 'running' : hasLearningStarted ? 'completed' : 'waiting'} userAction="点击当前任务，先和小涟完成学习准备，再明确开始一次学习行动。" xiaolianAction={currentTask ? `我会先陪你看清「${currentTask.knowledgePointName}」的诊断重点、计划目标与已有记录。` : '需要先加载或生成真实计划。'} systemCapability="学习准备确认后，沿用现有流程记录 learning_started Evidence，并携带真实 plan/task/knowledge point 上下文进入空间。">{currentTask && plan.plan ? <div className="flex flex-wrap items-center justify-between gap-4 rounded-[18px] border border-violet-100 bg-white/50 p-4"><div><p className="text-xs text-[var(--em-muted-ink)]">任务 {currentTask.order} / {plan.plan.tasks.length}</p><strong className="mt-1 block">{currentTask.knowledgePointName}</strong><p className="mt-1 text-xs text-[var(--em-muted-ink)]">点击先展示真实学习准备信息，确认后才记录开始学习证据。</p></div><Button onClick={() => setEntryOpen(true)} disabled={startingTaskId === currentTask.id} className="gap-2 rounded-xl bg-primary-500">{startingTaskId === currentTask.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}{startingTaskId === currentTask.id ? '正在记录…' : '准备当前任务'}</Button></div> : <XiaolianMessage tone="notice">先生成或加载真实 Current Plan，才能进入对应任务。</XiaolianMessage>}{startError && <p className="mt-3 text-sm text-amber-700">{startError}</p>}</DemoStoryStep>
