...
export function HeroBanner({ profile, diagnosis, plan, loading, error, runtimeState, companionState, onPrepareTask }: HeroBannerProps) {
  ...
  return <GlassPanel className="relative min-h-[500px] overflow-hidden p-5 sm:p-8 lg:p-10">
    ...
    <div className="relative grid items-center gap-4 sm:min-h-[430px] sm:gap-6 lg:grid-cols-[minmax(18rem,.8fr)_minmax(0,1.2fr)]">
      <XiaolianCharacter runtimeState={runtimeState} companionState={companionState} size="hero" priority />
      <div className="text-center lg:text-left"><p className="inline-flex items-center gap-2 rounded-full border border-violet-100 bg-white/65 px-3 py-1 text-xs font-semibold text-primary-700"><Sparkles className="h-3.5 w-3.5 text-companion" />{runtimeState === 'idle' ? COMPANION_TEXT[companionState] : RUNTIME_TEXT[runtimeState]}</p><h1 className="mt-4 text-[2rem] font-bold leading-tight sm:mt-5 sm:text-5xl">今天，我陪你从下一步开始</h1><p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[var(--em-muted-ink)] sm:mt-5 lg:mx-0">{observation}</p><div className="mt-4 rounded-[22px] border border-white/70 bg-white/55 p-4 text-left backdrop-blur-xl sm:mt-5"><p className="text-[10px] font-bold tracking-[0.16em] text-primary-600">CURRENT SUGGESTION</p><p className="mt-2 text-sm font-semibold leading-6">{suggestion}</p></div><div className="mt-5 flex flex-wrap justify-center gap-3 sm:mt-7 lg:justify-start">{nextTask ? <Button type="button" onClick={onPrepareTask} className="h-11 gap-2 rounded-2xl bg-primary-500 px-4 sm:px-5"><PlayCircle className="h-4 w-4" />开始当前任务</Button> : <Button asChild className="h-11 gap-2 rounded-2xl bg-primary-500 px-4 sm:px-5"><Link to="/my-learning"><PlayCircle className="h-4 w-4" />查看我的学习</Link></Button>}<Button asChild variant="outline" className="h-11 gap-2 rounded-2xl border-violet-200 bg-white/70 px-4 sm:px-5"><Link to="/xiaolian"><Bot className="h-4 w-4" />和小涟聊聊<ArrowRight className="h-4 w-4" /></Link></Button></div></div>
    </div>
  </GlassPanel>;
}
