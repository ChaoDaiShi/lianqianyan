...
export function XiaolianDailyInsight({ profile, diagnosis, plan, evidence, loading, error, onPrepareTask }: XiaolianDailyInsightProps) {
  ...
  return <GlassPanel className="p-5 sm:p-7">
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-3">
      ...
      <div className="rounded-[22px] border border-pink-100 bg-white/55 p-4"><p className="flex items-center gap-2 text-xs font-semibold text-fuchsia-700"><Compass className="h-4 w-4" />下一步建议</p>{loading ? <p className="mt-3 text-sm text-[var(--em-muted-ink)]">正在同步当前计划…</p> : nextTask ? <><strong className="mt-3 block">{nextTask.knowledgePointName}</strong><p className="mt-1 text-xs leading-5 text-[var(--em-muted-ink)]">当前计划任务 {nextTask.order}，预计 {nextTask.estimatedMinutes} 分钟。</p></> : <p className="mt-3 text-sm leading-6 text-[var(--em-muted-ink)]">当前没有 ACTIVE 学习计划，可以前往“我的学习”生成。</p>}{nextTask ? <Button type="button" variant="link" onClick={onPrepareTask} className="mt-3 h-auto gap-1 p-0 text-xs font-semibold text-primary-700">准备建议任务<ArrowRight className="h-3.5 w-3.5" /></Button> : <Link to="/my-learning" className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary-700">查看我的学习<ArrowRight className="h-3.5 w-3.5" /></Link>}</div>
    </div>
