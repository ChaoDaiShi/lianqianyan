...
export function KnowledgeStarMap({ points, primaryFocusId, selectedId, onSelect }: { points: KnowledgePointDiagnosis[]; primaryFocusId?: string | null; selectedId?: string | null; onSelect?: (point: KnowledgePointDiagnosis) => void }) {
  return (
    <div>
      ...
      <ol className="relative grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {points.map((point, index) => { const assessed = isAssessedDiagnosis(point.status); const tone = getDiagnosisTone(point.status); const primary = point.knowledgePointId === primaryFocusId; return (
          <li key={point.knowledgePointId} className="relative">
            <button type="button" onClick={() => onSelect?.(point)} aria-pressed={selectedId === point.knowledgePointId} aria-label={`${point.knowledgePointName}，${tone.label}，${point.evidenceCount} 条证据${assessed ? `，掌握度 ${formatDiagnosisPercent(point.masteryScore, true)}，可信度 ${formatDiagnosisPercent(point.confidence, true)}` : ''}`} className={cn('relative flex w-full gap-4 overflow-hidden rounded-[20px] border bg-white/60 p-4 text-left transition md:min-h-36 md:flex-col md:justify-between', primary ? 'border-fuchsia-200 shadow-[0_16px_40px_rgba(217,70,239,0.14)]' : 'border-violet-100 hover:border-primary-200', selectedId === point.knowledgePointId && 'ring-2 ring-primary-300 ring-offset-2')}>
