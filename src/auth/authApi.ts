...
export async function loginAccount(username: string, password: string): Promise<AuthAccount> {
  const response = await api.post<SessionWire>('/api/auth/login', { username, password });
  return mapAccount(response.data.account);
}
